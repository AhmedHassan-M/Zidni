...
$(document).ready(function () {
    ...
    $(document).on('click','.addOtherQuestions', function() {
        ...
        newQuiz.find(".quiz-questions-label").attr("for", "quizQuestionsTitle_" + (++quizQuestionsTitleLabel))

        // quizQuestionsTitle

        newQuiz.find(".quiz-questions-title").attr("id", "quizQuestionsTitle_" + (++quizQuestionsTitleId))
        newQuiz.find(".quiz-questions-title").attr("name", "quizQuestionsTitle_" + (++quizQuestionsTitleName))


        // quizQuestionsScoreLabel

        newQuiz.find(".quiz-questions-label").attr("for", "quizQuestionsScore_" + (++quizQuestionsScoreLabel))

        // quizQuestionsScore

        newQuiz.find(".quiz-questions-score").attr("id", "quizQuestionsScore_" + (++quizQuestionsScoreId))
        newQuiz.find(".quiz-questions-score").attr("name", "quizQuestionsScore_" + (++quizQuestionsScoreName))


        // quiz questions correct answers

        newQuiz.find(".quiz-questions-correct-answers").attr("name", "quizQuestionsCorrectAnswers_" + (++quizQuestionsCorrectAnswers) + "[]")

        // quiz questions answers title

        newQuiz.find(".quiz-questions-answers-title").attr("name", "quizQuestionsAnswersTitle_" + (++quizQuestionsAnswersTitle) + "[]")
    ...
    function radioError() {
        ...
        $('.pretty label.error').hover(function() {
            $(this).popover({
                ...
                content: 'Please check the correct answer for this question',
                placement: 'left'
              })
              .popover('show');
          });
    ...
    $(document).on('click','.addMoreAnswers', function() {
            ...
            const cloneAnswersFromQuestion = $(this).parent().find('fieldset .form-check').first().clone();
            ...
            let maxAnswersLimit = $(this).parent().find('fieldset .form-check')
            ...
            $('.quizQuestionsContainer').each(function(i, obj) {
                ...
                $(this).find('.quiz-questions-correct-answers').each(function(index) { 
    ...
    $(document).on('click','.quizQuestionsDelete', function() {
        ...
        let deleteQuestion =  $(this).parent().parent().parent().parent();
