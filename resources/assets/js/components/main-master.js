...
$(document).ready(function () {
  ...
  var player = new Vimeo.Player(iframe);
        ...
        let currentMaster = pathArray[3];

        console.log("current course :", currentCourse);

        $.ajax({
          type:'POST',
          url:'/update-progress/master-degree',
          data:{
              no:lessonNo,currentLess:currentLesson,currentCour:currentCourse,master:currentMaster
          },
                ...
                if(time == 0){
                    ...
                    for(var i = 0 ; i < data.length ; i++){
                      ...
                      if(data[i].id > currentLesson){
                          id = data[i].id;
                          window.location.replace("/master-degree/course-content/"+currentMaster+"/"+currentCourse+"/"+id);
                      }
                    }
  ...
  $("#activeQuizTab").one("click", function () {
    ...
    let quizForm
    const mainQuizForm = $("#masterQuizForm");
    ...
    mainQuizForm.on('sf-finish', function (e, from, data) {
        ...
        let quizForm = $('#masterQuizForm');

        $.ajax({

            type: "POST",
            url: "/answer-master-quiz",
            data: quizForm.serialize(),
            dataType: 'json',
            beforeSend: function () {
